...
class MyParser {
   ...
   static BufferedWriter item_table_buffer;
   static BufferedWriter bid_table_buffer;
   static BufferedWriter location_item_buffer;
   static BufferedWriter categ_table_buffer;
   static BufferedWriter bider_table_buffer;
   static BufferedWriter seller_table_buffer;
   ...
   public static void generateCSV(Node n) throws IOException, ParseException
   {
	   ...
	   Vector<String> Category=new Vector<String>(), RatingU =new Vector<String>(), UserIDU =new Vector<String>(), Time=new Vector<String>(), Amount=new Vector<String>(), Locationb=new Vector<String>(), Countryb=new Vector<String>();
	   ...
	   for (int i=0;i<category.length;i++)
	   {
		   String categ_tab = String.format("%s |*| %s\n", ItemID, Category.elementAt(i));
		   categ_table_buffer.write(categ_tab);
	   }
	   String item_tab = String.format("%s |*| %s |*| %s |*| %s |*| %s |*| %s |*| %s |*| %s |*| %s |*| %s\n", 
			   ItemID, Name, Currently, Buy_Price, First_Bid, Number_of_Bids, UserIDS, Started, Ends, Description);
	   item_table_buffer.write(item_tab);
	   
	   for (int i=0;i<bid.length;i++)
	   {
		   String bid_tab = String.format("%s |*| %s |*| %s |*| %s\n", ItemID, UserIDU.elementAt(i), Time.elementAt(i), Amount.elementAt(i));
		   bid_table_buffer.write(bid_tab);
		   String bider_tab = String.format("%s |*| %s |*| %s |*| %s\n", UserIDU.elementAt(i), RatingU.elementAt(i), Locationb.elementAt(i), Countryb.elementAt(i));
		   bider_table_buffer.write(bider_tab);
	   }
	   String location_item_tab = String.format("%s |*| %s |*| %s |*| %s |*| %s\n", ItemID, Longitute, Latitute, Location, Country);
	   location_item_buffer.write(location_item_tab);
	   String seller_tab = String.format("%s |*| %s\n", UserIDS, RatingS);
	   seller_table_buffer.write(seller_tab);
   }
   ...
   public static void main (String[] args){
       ...
       try {
           ...
           item_table = new FileWriter("item.dat");
           item_table_buffer = new BufferedWriter(item_table);
		   bid_table = new FileWriter("bid.dat");
		   bid_table_buffer = new BufferedWriter(bid_table);
		   location_item = new FileWriter("location_item.dat");
		   location_item_buffer = new BufferedWriter(location_item);
		   categ_table = new FileWriter("categ.dat");
		   categ_table_buffer = new BufferedWriter(categ_table);
		   bider_table = new FileWriter("bider.dat");
		   bider_table_buffer = new BufferedWriter(bider_table);
		   seller_table = new FileWriter("seller.dat");
		   seller_table_buffer = new BufferedWriter(seller_table);
		   
	       for (int i = 0; i < args.length; i++) {
	           File currentFile = new File(args[i]);
	           processFile(currentFile);
	       }
	       item_table_buffer.close();
	       item_table.close();
	       bid_table_buffer.close();
	       bid_table.close();
	       location_item_buffer.close();
	       location_item.close();
	       categ_table_buffer.close();
	       categ_table.close();
	       bider_table_buffer.close();
	       bider_table.close();
	       seller_table_buffer.close();
	       seller_table.close();
       }
